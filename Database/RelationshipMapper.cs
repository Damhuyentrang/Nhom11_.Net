...
namespace BTL_nhom11_marketPC.Database
{
    public class RelationshipMapper
    {
        ...
        {
            string query = @"INSERT INTO OCung (MaOCung, TenOCung, LoaiOCung, DungLuong, MoTa)
                            VALUES (@MaOCung, @TenOCung, @LoaiOCung, @DungLuong, @MoTa)";

            using (SqlConnection conn = DatabaseContext.GetConnection())
            using (SqlCommand cmd = new SqlCommand(query, conn))
            {
                cmd.Parameters.AddWithValue("@MaOCung", hardDrive.MaOCung);
                cmd.Parameters.AddWithValue("@TenOCung", hardDrive.TenOCung ?? (object)DBNull.Value);
                cmd.Parameters.AddWithValue("@LoaiOCung", hardDrive.LoaiOCung ?? (object)DBNull.Value);
                cmd.Parameters.AddWithValue("@DungLuong", hardDrive.DungLuong);
                cmd.Parameters.AddWithValue("@MoTa", hardDrive.MoTa ?? (object)DBNull.Value);
                ...
                catch (SqlException ex)
                {
                    if (ex.Number == 2627) // Lỗi trùng khóa chính
                    {
                        MessageBox.Show("Mã ổ cứng đã tồn tại!", "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        return false;
                    }
                    throw;
                }
        ...
        // Cập nhật ổ cứng
        public bool UpdateHardDrive(HardDrive hardDrive)
        {
            string query = @"UPDATE OCung SET TenOCung = @TenOCung, LoaiOCung = @LoaiOCung,
                            DungLuong = @DungLuong, MoTa = @MoTa
                            WHERE MaOCung = @MaOCung";
        ...
        // Xóa ổ cứng
        public bool DeleteHardDrive(string maOCung)
        {
            string query = "DELETE FROM OCung WHERE MaOCung = @MaOCung";

            using (SqlConnection conn = DatabaseContext.GetConnection())
            using (SqlCommand cmd = new SqlCommand(query, conn))
            {
                cmd.Parameters.AddWithValue("@MaOCung", maOCung);
                ...
                catch (SqlException ex)
                {
                    if (ex.Number == 547) // Lỗi khóa ngoại
                    {
                        MessageBox.Show("Không thể xóa ổ cứng do có máy tính đang sử dụng!", "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        return false;
                    }
                    throw;
                }
        ...
        public List<HardDrive> SearchHardDrives(string keyword)
        {
            var hardDrives = GetHardDrived();
            if (string.IsNullOrEmpty(keyword)) return hardDrives;

            keyword = keyword.ToLower();
            return hardDrives.Where(h =>
                h.MaOCung.ToLower().Contains(keyword) ||
                h.TenOCung.ToLower().Contains(keyword)).ToList();
        }

        // CARD ĐỒ HỌA
        // thêm Card
        public bool AddGraphicsCard(GraphicsCard graphicsCard)
        {
            string query = @"INSERT INTO CardDoHoa (MaCard, TenCard) 
                     VALUES (@MaCard, @TenCard)";

            using (SqlConnection conn = DatabaseContext.GetConnection())
            using (SqlCommand cmd = new SqlCommand(query, conn))
            {
                cmd.Parameters.AddWithValue("@MaCard", graphicsCard.MaCard);
                cmd.Parameters.AddWithValue("@TenCard", graphicsCard.TenCard);
        ...
        // Cập nhật Card
        public bool UpdateGraphicsCard(GraphicsCard graphicsCard)
        {
            string query = @"UPDATE CardDoHoa SET TenCard = @TenCard 
                     WHERE MaCard = @MaCard";
        ...
        // Xóa Card
        public bool DeleteGraphicsCard(string maCard)
        {
            string query = "DELETE FROM CardDoHoa WHERE MaCard = @MaCard";

            using (SqlConnection conn = DatabaseContext.GetConnection())
            using (SqlCommand cmd = new SqlCommand(query, conn))
            {
                cmd.Parameters.AddWithValue("@MaCard", maCard);
